...
import { changeTrialDiscount } from '../../../priceSlice';
...
export default function DogsNumberToggle(): JSX.Element {
  ...
  const handleTrialDiscountChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    setChecked(event.target.checked);
    dispatch(changeTrialDiscount());
    dispatch(calculateTotalPrice());
  };
      ...
      <div>
        <MyFormControlLabel control={
            <Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: 28 } }}
              ...
              checked={checked}
              onChange={handleTrialDiscountChange}
